...
class AktuelleAusrichtung(Enum):
    FAEHRT_NACH_LINKS = 0.015
    FAEHRT_NACH_RECHTS = -0.015
    GERADE_AUS = 0

class CameraLabirynth:
    ...
    def get_aktuelle_ausrichtung(self):
        ...
        #cv2.imwrite(f'./data/image-{uuid.uuid4()}.jpeg', img)

        yellow_lower_hsv = np.array([10, 0, 100])
        yellow_upper_hsv = np.array([50, 255, 255])
        ...
        if yellow_pixel_right * SIGNIFIKANZ_FAKTOR > yellow_pixel_left:
            return AktuelleAusrichtung.FAEHRT_NACH_LINKS
        if yellow_pixel_left * SIGNIFIKANZ_FAKTOR > yellow_pixel_right:
